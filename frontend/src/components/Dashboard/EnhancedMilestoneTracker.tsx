...
export const EnhancedMilestoneTracker = ({
  ...
  const [localMilestones, setLocalMilestones] =
    useState<EnhancedMilestone[]>(milestones);
      ...
      setLocalMilestones(prev =>
        prev.map(milestone => {
          if (milestone.id === milestoneId) {
            const newStatus =
              milestone.status === "completed" ? "current" : "completed";
            console.log(
              `🔄 Changing milestone ${milestoneId} from ${milestone.status} to ${newStatus}`
            );
      ...
      setLocalMilestones(prev =>
        prev.map(milestone => {
          if (milestone.id === milestoneId) {
            ...
            console.log(
              `📊 Updating milestone ${milestoneId}: ${completedCount}/${totalSteps} = ${Math.round(progress)}%`
            );
  ...
  useEffect(() => {
    console.log(
      "🔄 Completed steps changed, updating all milestone progress..."
    );
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <div className="flex items-center gap-1">
              ...
              <span>
                {milestones.filter(m => m.status === "current").length} in
                progress
              </span>
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <Badge
                          className={`${getStatusColor(milestone.status)} flex items-center gap-1`}
                        >
                          {getStatusIcon(milestone.status)}
                          {milestone.status.replace("-", " ").toUpperCase()}
                        </Badge>
                        <Badge
                          className={`${getPriorityColor(milestone.priority)} flex items-center gap-1`}
                        >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div className="flex items-center gap-2 text-slate-600">
                    ...
                    <div>
                      ...
                      <div
                        className={`font-bold ${isOverdue ? "text-red-600" : "text-slate-900"}`}
                      >
                        {milestone.dueDate}
                      </div>
                      {isOverdue ? (
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {milestone.steps.map((step, stepIndex) => (
                        <div
                          key={step.id}
                          className="p-4 rounded-lg border border-slate-200 hover:border-primary/30 transition-colors duration-300"
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              {step.requirements.length > 0 && (
                                <div className="mb-3">
                                  <p className="text-xs font-medium text-slate-700 mb-2 block">
                                    Requirements:
                                  </p>
                              ...
                              {step.notes && (
                                <div className="p-3 bg-slate-50 rounded-lg">
                                  <p className="text-xs font-medium text-slate-700 mb-1 block">
                                    Notes:
                                  </p>
